...
data3effLin <- function(N, J, beta, cov, sbruit){ ## Simulation des donn?es
  ...
  # On construit la variable reponse
  y = x %*% beta + z1 %*% rdm_effects[,1] + z2 %*% rdm_effects[,2] + z3 %*% rdm_effects[,3] + epsilon
...
stime <- system.time({
  resTotal <- foreach(k=1:K, .packages = c("lme4", "stats", "dplyr", "MASS", "varTestnlme")) %dopar% {
    ...
    for (c1 in cov1){
      for (c12  in cov12){
        set.seed(k*10000)
        cov0 = matrix(c(1.3, 0 , c12*sqrt(1.3*c1),
                        0, 0, 0,
                        c12*sqrt(1.3*c1),0, c1),nrow =3, byrow = TRUE)
        ...
        #resultat[[paste("v2 = ", c1, ", v12=", c12)]]$lrt_boot = lrt_B
...
#3) les paramètres pour générer les echantillons bootstrap 
param_sauv = matrix(0, nrow = K, ncol = 7) # beta de dim 2, gamma1, gamma2, gamma12 et sigma
...
for (r in rnam){
  ...
  #resultat[[r]]$lrt_boot = lrt_b_sauv
}
...
print(xtable(resultat$level_boot, type = "latex"), file = "puissance_boot.tex")
print(xtable(resultat$level_asym, type = "latex"), file = "puissance_asym.tex")
saveRDS(resultat, "power_m3_out.rda")
